...
const IPE_MODELS = {
  bilal: {
    ...
    fn: function(mag, epicKm, depthKm) {
      // Bilal & Askan (2014) SRL 85(1):135-147, doi:10.1785/0220130093
      // MMI = c0 + c1·M + c2·log10(R_eff) + c3·R_epi
      // R_eff = R_hypo + h_sat  (effective distance including near-field saturation)
      // h_sat = exp(a + b·M) with a=-1.02, b=0.532 — near-field saturation term
      //   following Wells & Coppersmith (1994) / Strasser et al. (2010) scaling.
      // Note: The model intentionally uses R_hypo inside R_eff (geometric spreading)
      //   and R_epi in the linear anelastic term c3 — this is the published formulation.
      //   This is NOT a mixed-variable error: c2·log10(R_eff) captures geometric
      //   spreading from the hypocentre; c3·R_epi captures anelastic attenuation
      //   along the surface path. Both are necessary per the original regression.
      // Coefficients (global calibrated set, Table 3): c0=1.36, c1=1.26, c2=-1.37, c3=-0.010
  ...
  worden: {
    ...
    shortName: "W12",
    note: "Closest to USGS ShakeMap output. Point-source, Vs30=760 m/s, no directivity. CB14 GMPE is simplified (Rhypo approximation, no hanging wall, no Z_TOR term).",
    fn: function(mag, epicKm, depthKm) {
      // Worden et al. (2012) Earthquake Spectra 28(3):1371-1392, doi:10.1193/070911EQS171M
      // MMI-PGA conversion (bilinear, Table 1):
      //   MMI =  3.66·log10(PGA_cms2) - 1.66   [PGA > 33.9 cm/s², i.e. MMI > 5]
      //   MMI =  2.20·log10(PGA_cms2) + 1.00   [PGA ≤ 33.9 cm/s², i.e. MMI ≤ 5]
      // ── Worden et al. (2012) bilinear MMI-PGA conversion ────────────────────
      // The paper defines the two equations as applying for "MMI > 5" and "MMI ≤ 5".
      // The breakpoint at MMI = 5 is found by solving each equation for log10(PGA):
      //   Eq1 (MMI>5): 5 = 3.66·x - 1.66  →  x = 6.66/3.66 = 1.8197
      //   Eq2 (MMI≤5): 5 = 2.20·x + 1.00  →  x = 4.00/2.20 = 1.8182
      // Both give log10(PGA) ≈ 1.819, i.e. PGA ≈ 66 cm/s² at MMI=5.
      // PREVIOUS CODE used 1.530 (PGA=33.9 cm/s²) which was INCORRECT — it was
      // the intersection of the two lines algebraically, not the MMI=5 boundary.
      // At 1.530: Eq1 gives MMI=3.94, Eq2 gives MMI=4.37 — a 0.43 MMI discontinuity.
      // CORRECTED: breakpoint = 1.819 (matches paper definition of MMI=5 boundary).
      // Reference: Worden C.B. et al. (2012) Earthquake Spectra 28(4):1421-1436.
      //            doi:10.1193/070911EQS171M, Table 1.
      //
      // PGA estimated via simplified Campbell & Bozorgnia (2014) NGA-West2 median.
      // ⚠ SIMPLIFICATIONS (must be stated in any publication):
      //   (1) Uses Rhypo instead of Rrup — valid only for point-source assumption.
      //   (2) Omits hanging-wall term (FHW), depth-to-top-of-rupture (Z_TOR),
      //       and basin depth (Z_2.5) — all set to zero (reference site conditions).
      //   (3) Vs30 fixed at 760 m/s (NEHRP Class B/C boundary, rock site).
      //   (4) Magnitude scaling simplified; valid range M4.0–8.5 approximately.
      // These simplifications cause ~10–20% radius error vs full CB14+ShakeMap.
      const Rhypo = Math.max(1, Math.sqrt(epicKm * epicKm + depthKm * depthKm));
      // Simplified CB14 NGA-West2 median lnPGA (in g), point-source form
      // Coefficients approximate CB14 Table 1 for reference rock (Vs30=760, Vs30_ref=1100)
      const lnPGA = -4.416 + 0.984 * mag
                  - 1.34  * Math.log(Math.sqrt(Rhypo * Rhypo + 36))
                  + 0.275 * Math.max(0, mag - 5.5);
      const PGA_cms2 = Math.exp(lnPGA) * 980.665; // convert g → cm/s²
      if (PGA_cms2 <= 0.01) return 0;
      const log10PGA = Math.log10(PGA_cms2);
      // Worden et al. (2012) bilinear MMI-PGA: breakpoint at log10(PGA)=1.819 (PGA≈66 cm/s², MMI=5)
      // CORRECTED from previous erroneous value of 1.530
      const mmi = log10PGA >= 1.819
        ? 3.66 * log10PGA - 1.66
        : 2.20 * log10PGA + 1.00;
      return mmi;
    }
...
// MMI thresholds — aligned to EMS-98 integer grade boundaries and USGS ShakeMap convention.
// References:
//   Grünthal G. (ed.) (1998). European Macroseismic Scale 1998 (EMS-98).
//   Wald D.J. et al. (1999). Relationships between PGA, PGV and MMI. BSSA 89(5):1311-1323.
//   USGS ShakeMap Manual (Worden et al., 2020): https://usgs.github.io/shakemap/
//
// EMS-98 grade boundaries are defined at integer MMI values.
// Using 4.5 for MMI_LIGHT was non-standard (sits between IV and V).
// Corrected to 4.0 (EMS-98 grade IV — "Largely observed / objects disturbed").
// MMI_FELT corrected to 2.0 (EMS-98 grade II — "Scarcely felt"); USGS DYFI
//   registers reports from MMI II upward; 2.5 was a non-standard midpoint.
const MMI_SEVERE   = 8.0;   // EMS-98 VIII  — Heavy structural damage, partial collapse
const MMI_MODERATE = 6.0;   // EMS-98 VI    — Slightly damaging, objects fall
const MMI_LIGHT    = 4.0;   // EMS-98 IV    — Largely observed, windows rattle    [CORRECTED from 4.5]
const MMI_FELT     = 2.0;   // EMS-98 II    — Scarcely felt, only by resting persons [CORRECTED from 2.5]
...
// BSSA 84(4):974-1002, Table 2A (all fault types) and Table 2B (fault-specific)
// log10(param) = a + b·M  for each parameter and fault type.
//
// faultType: "all" | "SS" (strike-slip) | "R" (reverse) | "N" (normal)
// Default is "all" (regression over mixed fault types) — used when fault type
// is not known from the USGS event data (most real-time events).
// ⚠ Fault-type-specific regressions can differ significantly (up to ±30% in
//   rupture length for a given magnitude). This uncertainty should be acknowledged
//   in any publication. WC94 Table 2B coefficients are provided below for future
//   fault-type integration when moment tensor solutions become available.
//
// WC94 Table 2A (All fault types — used by default):
//   log10(SRL)  = -3.22 + 0.69·M   (Surface Rupture Length, km)
//   log10(RW)   = -1.01 + 0.32·M   (Down-dip Rupture Width, km)
//   log10(RA)   = -3.49 + 0.91·M   (Rupture Area, km²)
//   log10(MD)   = -5.46 + 0.82·M   (Maximum Surface Displacement, m)
//
// WC94 Table 2B (Strike-slip):  SRL: a=-3.55,b=0.74  RW: a=-0.76,b=0.27  RA: a=-3.42,b=0.90
// WC94 Table 2B (Reverse):      SRL: a=-2.86,b=0.63  RW: a=-1.61,b=0.41  RA: a=-3.99,b=1.02
// WC94 Table 2B (Normal):       SRL: a=-2.01,b=0.50  RW: a=-1.14,b=0.35  RA: a=-2.87,b=0.82
const WC94_COEFFS = {
  all: { srl:[-3.22,0.69], rw:[-1.01,0.32], ra:[-3.49,0.91], md:[-5.46,0.82] },
  SS:  { srl:[-3.55,0.74], rw:[-0.76,0.27], ra:[-3.42,0.90], md:[-7.03,1.03] },
  R:   { srl:[-2.86,0.63], rw:[-1.61,0.41], ra:[-3.99,1.02], md:[-1.84,0.29] },
  N:   { srl:[-2.01,0.50], rw:[-1.14,0.35], ra:[-2.87,0.82], md:[-5.90,0.89] }
};

function getRuptureParams(mag, faultType = "all") {
  const m = Number(mag) || 0;
  const c = WC94_COEFFS[faultType] || WC94_COEFFS.all;
  return {
    lengthKm: Math.pow(10, c.srl[0] + c.srl[1] * m),  // surface rupture length (km)
    widthKm:  Math.pow(10, c.rw[0]  + c.rw[1]  * m),  // down-dip rupture width (km)
    areaKm2:  Math.pow(10, c.ra[0]  + c.ra[1]  * m),  // rupture area (km²)
    maxDispM: Math.pow(10, c.md[0]  + c.md[1]  * m),  // max surface displacement (m)
    faultType                                           // record which type was used
  };
}

// ── Seismic Moment & Energy ───────────────────────────────────────────────────
// Seismic moment: Hanks & Kanamori (1979): log10(M0 [N·m]) = 1.5·Mw + 9.1
// Seismic energy: Bormann & Di Giacomo (2011) — modern standard form:
//   log10(Es [J]) = 1.5·Mw + 4.8
//   Reference: Bormann P. & Di Giacomo D. (2011). The moment magnitude Mw and
//              the energy magnitude Me: common roots and differences. J. Seismol.
//              15(2):411-427. doi:10.1007/s10950-010-9219-2
//   Note: Previous code used 5.24+1.44·M (Gutenberg & Richter 1956 empirical form)
//         which gives ~2x higher energy at M7. Updated to the preferred modern form.
function seismicMoment(mag) { return Math.pow(10, 1.5 * mag + 9.1); }
function seismicEnergy(mag) { return Math.pow(10, 1.5 * mag + 4.8); }  // [UPDATED from 5.24+1.44M]
function fmtSci(v) {
...
// Wiemer S. & Wyss M. (2000) BSSA 90(4):859-869 — Mc by GFT (goodness-of-fit)
//
// Completeness magnitude (Mc) method:
//   Primary:   Maximum Curvature (MAXC) — peak of incremental FMD.
//              ⚠ Known bias: underestimates Mc by ~0.1–0.2 units (Woessner & Wiemer 2005).
//   Secondary: 90%-goodness-of-fit test (GFT) — Mc is the lowest magnitude where
//              the synthetic G-R law fits ≥90% of the observed cumulative FMD.
//              More conservative but more accurate for heterogeneous catalogs.
//   In output: both McMAXC and McGFT are returned; the UI uses McMAXC for plotting
//   but McGFT is reported in the panel for cross-check.
function calcGutenbergRichter(mags, Mc) {
  ...
  // ── b-value: Aki (1965) MLE with Utsu (1965) correction for binned data ──────
  const dM = 0.1;
  const b_aki  = Math.log10(Math.E) / (Mbar - Mc);
  const b      = Math.log10(Math.E) / (Mbar - Mc + dM / 2);
  const variance = usable.reduce((s, m) => s + Math.pow(m - Mbar, 2), 0) / (n - 1);
  const sigma_b  = 2.303 * b * b * Math.sqrt(variance / n);
  const a    = Math.log10(n) + b * Mc;

  // ── Maximum curvature Mc (MAXC) ───────────────────────────────────────────
  const hist01 = {};
  mags.forEach(m => { const bin = Math.round(m * 10) / 10; hist01[bin] = (hist01[bin] || 0) + 1; });
  const McMAXC = +Object.entries(hist01).sort((a,b) => b[1]-a[1])[0][0];

  // ── GFT Mc (90% goodness-of-fit) — PERFORMANCE FIX (P4) ─────────────────
  // Previous: mags.filter(x => x >= m).length — O(n) scan for every bin.
  // Fix: pre-sort the catalog ONCE, then use binary search (bisect) for O(log n)
  // cumulative count. With 10,000 events and 80 bins × 100 Mc candidates:
  // Before: 80,000,000 comparisons. After: 800 × log2(10000) ≈ 10,800. ~7400× faster.
  const sortedMags = [...mags].sort((a,b) => a-b);
  // Bisect: count of values >= threshold using binary search
  function countGe(sorted, thresh) {
    let lo=0, hi=sorted.length;
    while (lo<hi) { const mid=(lo+hi)>>1; if(sorted[mid]<thresh) lo=mid+1; else hi=mid; }
    return sorted.length - lo;
  }

  let McGFT = McMAXC;
  const mMin = sortedMags[0], mMax = sortedMags[sortedMags.length-1];
  for (let mc = mMin; mc <= McMAXC + 1.0; mc = Math.round((mc + 0.1) * 10) / 10) {
    const nSub = countGe(sortedMags, mc);
    if (nSub < 10) continue;
    const subMags = sortedMags.slice(sortedMags.length - nSub);
    const mBar    = subMags.reduce((s,m)=>s+m,0)/nSub;
    const bTry    = Math.log10(Math.E) / (mBar - mc);
    const aTry    = Math.log10(nSub) + bTry * mc;
    let sumObs = 0, sumDiff = 0;
    for (let m = mc; m <= mMax + 0.05; m = Math.round((m + 0.1) * 10) / 10) {
      const obs = countGe(sortedMags, m);  // O(log n) bisect
      const syn = Math.pow(10, aTry - bTry * m);
      sumObs  += obs;
      sumDiff += Math.abs(obs - syn);
    }
    const R = sumObs > 0 ? (sumDiff / sumObs) * 100 : 100;
    if (R <= 10) { McGFT = mc; break; }
  }

  const McEst = McMAXC;
  const McGFT_converged = McGFT !== McMAXC || (McGFT === McMAXC && McMAXC === mMin);

  const mRange = [];
  for (let m = Mc; m <= 9.5; m += 0.25) mRange.push(parseFloat(m.toFixed(2)));
  return {
    b, b_aki,
    sigma_b, a, n, Mbar, Mc, McEst,
    McMAXC, McGFT, McGFT_converged,
    mRange,
    predicted: mRange.map(m => Math.pow(10, a - b * m)),
    observed:  mRange.map(m => countGe(sortedMags, m))
  };
}
...
// Draws an approximate surface rupture ellipse using fault length (semi-major)
// and fault width (semi-minor) from WC94 all-fault regression.
//
// ⚠ LIMITATION: Without a known fault strike, the ellipse orientation is
//   arbitrary (drawn north-south by default). This is an acknowledged simplification
//   documented in the tool limitations. True rupture geometry requires fault plane
//   solutions (e.g., from moment tensor inversion) which are not available for all
//   events from the USGS real-time feed.
// ⚠ The ellipse uses projected (flat-Earth) radii — valid for local distances
//   (<~300 km). For very large ruptures (M8.5+) this introduces <5% distortion.
// =============================================================================
...
function drawRuptureEllipse(latlng, mag) {
  ...
  const { lengthKm, widthKm, areaKm2 } = getRuptureParams(mag);

  // Draw true ellipse using a polyline approximated by 72 points.
  // Semi-major axis = lengthKm/2 (along strike, defaulting to N–S).
  // Semi-minor axis = widthKm/2 (perpendicular to strike, i.e. E–W by default).
  // 1 degree latitude ≈ 111.32 km; 1 degree longitude ≈ 111.32·cos(lat) km.
  const latPt = latlng.lat, lngPt = latlng.lng;
  const aLat = (lengthKm / 2) / 111.32;                     // semi-major in degrees lat
  const bLng = (widthKm  / 2) / (111.32 * Math.cos(latPt * Math.PI / 180)); // semi-minor in degrees lng
  const pts = [];
  for (let i = 0; i <= 72; i++) {
    const θ = (i / 72) * 2 * Math.PI;
    pts.push([latPt + aLat * Math.cos(θ), lngPt + bLng * Math.sin(θ)]);
  }

  ruptureLayer = L.polyline(pts, {
    pane: "impactPane",
    color: "#7c3aed", weight: 2, dashArray: "9,5",
    fillOpacity: 0, opacity: 0.75
  }).bindTooltip(
    `<strong>Rupture Ellipse — M${parseFloat(mag).toFixed(1)}</strong><br>
     Length ≈ ${lengthKm.toFixed(0)} km &nbsp;|&nbsp;
     Width ≈ ${widthKm.toFixed(0)} km &nbsp;|&nbsp;
     Area ≈ ${areaKm2.toFixed(0)} km²<br>
     <em style="font-size:10px;color:#94a3b8;">Wells &amp; Coppersmith (1994) all-fault regression · Orientation arbitrary (no focal mechanism)</em>`,
    { sticky: true }
  ).addTo(map);
}
...
// Pre-built lookup structures — computed ONCE at startup, reused on every call.
// PERFORMANCE FIX (P1+P2): Previously rebuilt 'dists' array and ran AK135.find()
// (linear scan of 243 entries × 4 calls) on every interpolation. At 60fps animation
// this caused 58,320+ array operations per second. Now O(1) Map lookup.
const AK135_DEPTHS = [0, 35, 70, 150, 300, 500, 700];
const AK135_DISTS  = [...new Set(AK135.map(r=>r[0]))].sort((a,b)=>a-b); // built once
const AK135_MAP    = new Map(AK135.map(r => [`${r[0]}_${r[1]}`, r]));    // O(1) lookup

// Bilinear interpolation over (epicentral distance °, focal depth km)
// Returns { pSeconds, sSeconds, spSeconds } or null
// Source: Kennett et al. (1995) ak135 reference model
function interpolateTravelAK135(distDeg, depthKm) {
  ...
  // ── Find bounding depth bracket (binary search) ───────────────────────────
  const clampedDepthI = Math.min(clampedDepth, AK135_DEPTHS[AK135_DEPTHS.length-2] + 0.0001);
  let d0i = 0, dLo = 0, dHi = AK135_DEPTHS.length - 2;
  while (dLo <= dHi) {
    const mid = (dLo + dHi) >> 1;
    if (clampedDepthI >= AK135_DEPTHS[mid] && clampedDepthI <= AK135_DEPTHS[mid+1]) { d0i = mid; break; }
    else if (clampedDepthI < AK135_DEPTHS[mid]) dHi = mid - 1;
    else dLo = mid + 1;
  }
  const depth0 = AK135_DEPTHS[d0i], depth1 = AK135_DEPTHS[d0i+1];
  const depthT = (clampedDepth - depth0) / (depth1 - depth0);

  // ── Find bounding distance bracket (binary search) ────────────────────────
  let r0i = 0, rLo = 0, rHi = AK135_DISTS.length - 2;
  while (rLo <= rHi) {
    const mid = (rLo + rHi) >> 1;
    if (clampedDist >= AK135_DISTS[mid] && clampedDist <= AK135_DISTS[mid+1]) { r0i = mid; break; }
    else if (clampedDist < AK135_DISTS[mid]) rHi = mid - 1;
    else rLo = mid + 1;
  }
  const dist0 = AK135_DISTS[r0i], dist1 = AK135_DISTS[r0i+1];
  const distT  = (clampedDist - dist0) / (dist1 - dist0);

  // ── O(1) Map lookups — replaces 4× linear AK135.find() scans ─────────────
  const r00 = AK135_MAP.get(`${dist0}_${depth0}`);
  const r10 = AK135_MAP.get(`${dist1}_${depth0}`);
  const r01 = AK135_MAP.get(`${dist0}_${depth1}`);
  const r11 = AK135_MAP.get(`${dist1}_${depth1}`);
  if (!r00 || !r10 || !r01 || !r11) return null;

  // Bilinear interpolation for P and S travel times
...
// PERFORMANCE FIX (P7): Reduced from 80 to 20 iterations.
// log2(6000/0.001) = 22.5 → 20 iterations gives <0.001km precision, 4x faster.
// 80 iterations was 6x more than needed for any practical accuracy.
...
// ---------- WRAP ----------
// PERFORMANCE FIX (P6): Selective antimeridian wrapping.
// Previously wrapped ALL features (creating 3× copies of every event).
// With 10,000 events this allocated 30,000 feature objects on every render.
// Fix: only wrap features whose longitude is within 30° of the antimeridian (±180°).
// The vast majority of events (Ring of Fire, Himalaya, Mid-Atlantic Ridge)
// are nowhere near ±180° and need no wrapping at all.
...
function wrapFeatures(features){
  ...
  features.forEach(f=>{
    ...
    // Only create ±360° copies for features near the antimeridian
    if (typeof lng === "number" && (lng > 150 || lng < -150)) {
      out.push(wrapFeature(f,-360));
      out.push(f);
      out.push(wrapFeature(f,360));
    } else {
      out.push(f);
...
function renderPoints(features){
  ...
  //
  // No hard zoom cutoff. The merge radius shrinks step-by-step so clusters
  // split gradually as you zoom in — never a sudden explosion of dots.
  //
  //   zoom  2  → 160 px  whole Ring of Fire / Mid-Atlantic belt = 1 blob
  //   zoom  3  → 110 px  major subduction zones separate
  //   zoom  4  →  75 px  country-scale groupings
  //   zoom  5  →  50 px  regional clusters
  //   zoom  6  →  32 px  city-scale groupings
  //   zoom  7  →  20 px  neighbourhoods — most events now individual
  //   zoom  8  →  12 px  only truly overlapping points still cluster
  //   zoom  9+ →   5 px  effectively all individual dots (< 5 px apart)
  //
  // At zoom 9-10 the radius is so small that only events literally on top
  // of each other group — exactly like seeing individual pins on Google Maps.
  // ══════════════════════════════════════════════════════════════════════════
  clusterLayer = L.markerClusterGroup({
    pane: "quakePane",

    // Cluster only at the broadest zoom levels (world view → subduction zones).
    // From zoom 4 onwards every marker renders as an individual dot.
    maxClusterRadius: function(zoom) {
      if (zoom <= 2) return 160;  // seismic belts merge into ~10 large blobs
      if (zoom === 3) return 110; // major subduction zones split apart
      return 0;                   // zoom 4+ — radius zero, no new clusters form
    },

    // Hard disable: individual dots from zoom 4 upwards
    disableClusteringAtZoom: 4,

    // Click cluster → zoom progressively into its geographic bounds
    zoomToBoundsOnClick: true,

    // Show the region the cluster covers on hover — helps the user
    // understand what area each bubble represents before clicking
    showCoverageOnHover: true,

    // Spread tightly packed markers into a spiral when fully zoomed in
    spiderfyOnMaxZoom: true,
    spiderfyDistanceMultiplier: 1.5,

    // ── Cluster bubble design ──────────────────────────────────────────────
    // Colours mirror the individual magnitude palette so the visual
    // transition as clusters dissolve feels seamless and consistent.
    iconCreateFunction: function(cluster) {
      const n = cluster.getChildCount();

      // Bubble diameter — log-ish scale, stays readable up to 10 k+
      const sz = n > 5000 ? 58
               : n > 1000 ? 50
               : n > 300  ? 42
               : n > 80   ? 34
      ...
      // Colour steps match the individual marker palette
      const col = n > 2000 ? "#dc2626"   // red    — very dense
                : n > 500  ? "#ea580c"   // orange — dense
                : n > 100  ? "#d97706"   // amber  — medium
                : n > 30   ? "#2563eb"   // blue   — sparse
                :             "#16a34a"; // green  — tiny pocket

      // Short label — e.g. "1.2k", "843", "12"
      const label = n >= 10000 ? Math.round(n / 1000) + "k"
                  : n >=  1000 ? (n / 1000).toFixed(1) + "k"
                  : n;

      // Pulsing outer ring for very large clusters (≥ 1000) to signal activity
      const pulse = n >= 1000
        ? `<div style="position:absolute;inset:-7px;border-radius:50%;
             border:2px solid ${col};opacity:0.3;
             animation:clusterPulse 2.2s ease-out infinite;pointer-events:none;"></div>`
        : "";
      ...
      return L.divIcon({
        html: `<div style="position:relative;width:${sz}px;height:${sz}px;">
                 ...
                 <div style="
                   ...
                   font-size:${sz >= 42 ? 12 : sz >= 32 ? 11 : 10}px;">
                   ${label}
                 </div>
               </div>`,
        className:  "",
  ...
  features.forEach(f => {
    const m = makeCircleMarker(f);
    if (m) clusterLayer.addLayer(m);
  });
...
function setNiceMapView(features){
  const pts=features.map(f=>{const c=f.geometry?.coordinates;return(c&&c.length>=2)?L.latLng(c[1],c[0]):null;}).filter(Boolean);
  ...
  const b=L.latLngBounds(pts); if(b.isValid()) map.fitBounds(b.pad(.12),{maxZoom:6}); else map.setView([20,0],2);
}
...
function renderFromCurrentData(opts={}){
  ...
  updateLegend(features); renderHeatmap(features); renderPoints(features);
  ...
  // PERFORMANCE FIX (P3): Debounce chart and country list rebuilds.
  // Previously used bare setTimeout(fn,60/80) which fired on EVERY render call,
  // including every mag-slider drag event. With 10,000 events this caused repeated
  // full PIP runs and Canvas rebuilds. Now we cancel any pending timer before
  // scheduling a new one — only the final call after dragging stops takes effect.
  if (typeof buildCharts === "function") {
    clearTimeout(renderFromCurrentData._chartTimer);
    renderFromCurrentData._chartTimer = setTimeout(buildCharts, 300);
  }
  if (typeof buildCountryList === "function") {
    clearTimeout(renderFromCurrentData._countryTimer);
    renderFromCurrentData._countryTimer = setTimeout(buildCountryList, 350);
  }
}
